    ...
    const check_valid = async () => {
        if (contact_name != '' && contact_number != '') {
            ...
            alt({
                ...
                message: strings.please_enter_all_the_required_fields,
              });
        }
    }
        ...
        .then(async response => {
            ...
            if(response.data.status == 1){
                ...
                alt({
                    ...
                    title: strings.Your_emergency_contact_added_successfully,
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>{strings.add_emergency_contact}</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>{strings.add_emergency_contact_name_and_number}</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
                    ...
                    {loading == false ?
                        <TouchableOpacity onPress={check_valid.bind(this)} activeOpacity={1} style={{ width: '100%', backgroundColor: colors.btn_color, borderRadius: 10, height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                            <Text style={{fontSize: f_m, color: colors.theme_fg_three, fontFamily: bold }}>{strings.add}</Text>
                        </TouchableOpacity>
                        :
