...
const Splash = (props) => {
    ...
    const unsubscribe = addEventListener(state => {
      ...
      if(state.isConnected == true){
        ...
        dropDownAlertRef({
          ...
          title: strings.internet_connection_error,
          message: strings.please_enable_your_internet_connection,
        });
      }
    });
    unsubscribe();
  ...
  // Channel Create
   const channel_create = async () => {
      
    PushNotification.createChannel(
      {
        channelId: "Booking", // (required)
        channelName: "Booking_taxi", // (required)
        channelDescription: "Taxi Booking Solution", // (optional) default: undefined.
        playSound: true, // (optional) default: true
        soundName: "uber", // (optional) See `soundName` parameter of `localNotification` function
      ...
      (created) =>{
         ...
         if(!created) {
            return console.log(`Channel already exists or there was an error creating it.`);
         }
    ...
    await axios({
      ...
      .then(async response => {
        if(response.data.result.android_latest_version.version_code > app_version_code){
          navigate_update_app('https://play.google.com/store/apps/details?id=com.lyftu_customer.customer');
        }else{
