...
import { screenHeight, GOOGLE_KEY, screenWidth, normal, bold, app_name, sos, regular,f_xs, api_url, trip_details, img_url, get_tips, add_tip, trip_cancel, loader, sos_sms } from '../config/Constants';
...
const TripDetails = (props) => {
    ...
    useEffect(() => {
        call_get_tips();
        call_trip_details();
        // console.log("TripDetails page TripId", trip_id);
    
        // Firebase listener for trip updates
        const tripRef = database().ref(`/trips/${trip_id}`);
        const onValueChange = tripRef.on('value', snapshot => {
            const tripData = snapshot.val();
            if (tripData) {
                // console.log("onChange value", tripData);
                if (tripData.status !== data.status) {
                    call_trip_details();
                }
            } else {
                console.warn("Trip data is undefined");
            }
        });
    
        // Firebase listener for driver tracking updates
        const driverRef = database().ref(`/drivers/${data.trip.vehicle_type}/${data.trip.driver_id}`);
        const onDriverTracking = driverRef.on('value', snapshot => {
            const driverData = snapshot.val();
            if (driverData) {
                // console.log("onDriver tracking", driverData);
    
                const { lat, lng, bearing } = driverData.geo;
                const marker = {
                ...
                if (data.trip.status <= 2) {
                    get_direction(`${lat},${lng}`, `${data.trip.pickup_lat},${data.trip.pickup_lng}`);
                } else {
                    get_direction(`${lat},${lng}`, `${data.trip.drop_lat},${data.trip.drop_lng}`);
                }
                ...
                console.warn("Driver data is undefined");
            }
        });
    
        // Cleanup function to remove Firebase listeners
        return () => {
            tripRef.off('value', onValueChange);
            driverRef.off('value', onDriverTracking);
        };
    }, [trip_id, data]);
...
// Original data
    ...
    //     call_get_tips();
    //     call_trip_details();
    //     console.log("TripDetials page TripId",trip_id)
    //     const onValueChange = database().ref(`/trips/${trip_id}`)
    //         .on('value', snapshot => {
    //             console.log("onChange value",snapshot.val());
    ...
    //         .on('value', snapshot => {
    //             console.log("onDriver tracking", snapshot.val());
            ...
            .then(async response => {
                ...
                if (response.data.status == 1) {
                    alt({
                        ...
                        title: strings.thank_You,
                        message: strings.your_tip_added_successfully,
                      });
    ...
    const call_trip_details = () => {
        ...
        axios({
            ...
            .then(async response => {
                ...
                // console.log("data coming from call_trip_details :",response.data.result);
    ...
    const get_location = async () => {
        if (Platform.OS == "android") {
             console.log("requestCameraPermission is working")
            await requestCameraPermission();
        } else {
             console.log("get the initial location")
            await getInitialLocation();
        }
    }

// Function to request location permission and get initial location
    const requestCameraPermission = async () => {
        try {
            const granted = await PermissionsAndroid.request(
                PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, {
                'title': strings.location_access_required,
                'message': { app_name } + strings.Needs_to_access_your_location_for_tracking
            }
            )
 ...
 //SOS Alert Call or sms Function   
const call_sos_sms = (lat, lng) => {
    ...
    return (
        <View style={styles.container}>
                    ...
                    <ScrollView onScrollEndDrag={onScrollEndDrag} showsVerticalScrollIndicator={false}>
                        {on_load == 1 ?
                            <View style={{ padding: 10 }}>
                                <View>
                                    <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: 15, fontFamily: normal }}>{strings.your_otp_code_is} : #{data.trip.otp}</Text>
                                </View>
                                ...
                                <View style={{ borderBottomWidth: 0.5, borderColor: colors.grey }}>
                                    <View style={{ flexDirection: 'row', width: '100%', marginTop: 10, marginBottom: 10 }}>
                                        <View style={{ width: '33%', alignItems: 'center', justifyContent: 'center' }}>
                                            <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}>{strings.distance}</Text>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                        ...
                                        <View style={{ width: '33%', alignItems: 'center', justifyContent: 'center' }}>
                                            <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}>{strings.trip_type}</Text>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                        ...
                                        <View style={{ width: '33%', alignItems: 'center', justifyContent: 'center' }}>
                                            <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 13, fontFamily: regular }}>{strings.estimation_fare}</Text>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                        ...
                                        <TouchableOpacity activeOpacity={1} style={{ width: '100%', backgroundColor: colors.theme_bg_three }}>
                                            <View style={{ flexDirection: 'row', width: '100%', height: 50 }}>
                                                ...
                                                <View style={{ width: '90%', alignItems: 'flex-start', justifyContent: 'flex-start' }}>
                                                    <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 12, fontFamily: regular }}>{strings.pickup_address}</Text>
                                                    <View style={{ margin: 2 }} />
                                                    <Text numberOfLines={2} ellipsizeMode='tail' style={{ color: colors.theme_fg_two, fontSize: 13, fontFamily: regular }}>{data.trip.pickup_address}</Text>
                                                </View>
                                            </View>
                                        </TouchableOpacity>
                                        <View style={{ margin: 10 }} />
                                        {data.trip.trip_type != 2 &&
                                            <TouchableOpacity activeOpacity={1} style={{ width: '100%', backgroundColor: colors.theme_bg_three }}>
                                                <View style={{ flexDirection: 'row', width: '100%', height: 50 }}>
                                                    ...
                                                    <View style={{ margin: 3 }} />
                                                    <View style={{ width: '90%', alignItems: 'flex-start', justifyContent: 'flex-start' }}>
                                                        <Text numberOfLines={1} style={{ color: colors.grey, fontSize: 12, fontFamily: regular }}>{strings.drop_address}</Text>
                                                        <View style={{ margin: 2 }} />
                                                        <Text numberOfLines={2} ellipsizeMode='tail' style={{ color: colors.theme_fg_two, fontSize: 13, fontFamily: regular }}>{data.trip.drop_address}</Text>
                                                    </View>
                                                </View>
                                            </TouchableOpacity>
                                        }
                                ...
                                {data.trip.status >= 1 && data.trip.status <= 5 &&
                                    <View style={{ width: '100%', marginBottom: 20 }}>
                                        {tip == 0 &&
                                            <View style={{ padding: 10 }}>
                                                <Text style={{ color: colors.theme_fg_two, fontSize: 20, fontFamily: bold }}>{strings.add_a_tip_for_your_driver}</Text>
                                                <View style={{ margin: 2 }} />
                                                <Text style={{ color: colors.theme_fg_two, fontSize: 14, fontFamily: regular }}>{strings.the_entire_amount_will_be_transferred_to_the_rider_valid_only_if_you_pay_online}</Text>
                                                <View style={{ margin: 5 }} />
                                        ...
                                        <View style={{ flexDirection: 'row', width: '100%', marginBottom: 20 }}>
                                            ...
                                            {loading == false ?
                                                <TouchableOpacity onPress={showDialog.bind(this)} activeOpacity={1} style={{ width: '55%', backgroundColor: colors.error_background, borderRadius: 10, height: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                                                    <Text style={{ color: colors.theme_fg_two, fontSize: 16, color: colors.error, fontFamily: bold }}>{strings.cancel}</Text>
                                                </TouchableOpacity>
                                                :
                            ...
                            <View style={{ alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                                <Text style={{ color: colors.theme_fg_two, fontSize: 15, fontFamily: regular }}>{strings.loading}</Text>
                            </View>
                        }
                    </ScrollView>
                )}
            ...
            > 
                <Dialog.Title>
                    <Text style={{ color : colors.theme_fg_two }}>{strings.reason_to_cancel_your_ride}</Text>
                </Dialog.Title>
                <Dialog.Description>
                    <FlatList
