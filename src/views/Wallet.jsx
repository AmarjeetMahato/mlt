...
const Wallet = (props) => {
    ...
    const handleOnRedirect = (data) => {
        ...
        if (data.status == "successful") {
            ...
            alert(strings.sorry_your_payment_declined);
        }
        ...
        .then(async response => {
            ...
            if(response.data.status == 1){
                ...
                alt({
                    ...
                    message: strings.amount_successfully_added_to_your_walle,
                  });
                ...
                alt({
                    ...
                    title: strings.error,
                    message:strings.response_data_message,
                  });
        ...
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={[styles.header]}>
                ...
                <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontFamily: bold }}>{strings.wallet}</Text>
                </View>
            </View>
            <ScrollView>
                        ...
                        <View style={{ width: '100%', backgroundColor: colors.theme_bg_three, borderRadius: 10, padding: 20 }}>
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ width: '55%', alignItems: 'flex-start', justifyContent: 'center' }}>
                                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.text_grey, fontSize: f_s, fontFamily: normal }}>{strings.total_wallet_balance}</Text>
                                </View>
                                <TouchableOpacity activeOpacity={1} onPress={open_dialog} style={{ width: '30%', alignItems: 'flex-end', justifyContent: 'center' }}>
                                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg, fontSize: f_xs, fontFamily: normal }}>{strings.top_up}</Text>
                                </TouchableOpacity>
                ...
                <View style={{ padding: 20 }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.text_grey, fontSize: f_s, fontFamily: normal }}>{strings.transactions_list}</Text>
                    ...
                    <View style={{ flexDirection: 'row', width: '100%' }}>
                        <View style={{ width: '33%', alignItems: 'flex-start', justifyContent: 'center' }}>
                            <TouchableOpacity onPress={change_filter.bind(this, 1)} style={[filter == 1 ? styles.segment_active_bg : styles.segment_inactive_bg]}>
                                <Text style={[filter == 1 ? styles.segment_active_fg : styles.segment_inactive_fg]}>{strings.all}</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={{ width: '33%', alignItems: 'center', justifyContent: 'center' }}>
                            <TouchableOpacity onPress={change_filter.bind(this, 2)} style={[filter == 2 ? styles.segment_active_bg : styles.segment_inactive_bg]}>
                                <Text style={[filter == 2 ? styles.segment_active_fg : styles.segment_inactive_fg]}>{strings.expenses}</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={{ width: '33%', alignItems: 'flex-end', justifyContent: 'center' }}>
                            <TouchableOpacity onPress={change_filter.bind(this, 3)} style={[filter == 3 ? styles.segment_active_bg : styles.segment_inactive_bg]}>
                                <Text style={[filter == 3 ? styles.segment_active_fg : styles.segment_inactive_fg]}>{strings.receives}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                    <View style={{ margin: 10 }} />
            ...
            >
                ...
                {flutterwave_id == 1 &&
                    <View>
                        ...
                        />
                        <View style={{ margin: 10 }} />
                        <Text style={{ fontFamily: normal, color: colors.theme_fg_two, alignSelf: 'center', fontSize: 16 }} onPress={close_flutterwave}>{strings.cancel}</Text>
                    </View>
                }
            </RBSheet>
            ...
            <DialogInput isDialogVisible={isDialogVisible}
                 ...
                 message={strings.please_enter_your_amount}
